...
module.exports = {
  networks: Object.entries(hardhatNetworks).reduce((acc, [key, network]) => {
    // filter networks without specified private keys
    // without them hardhat fails
    if (key === "hardhat") {
      acc[key] = network
    } else {
      if (network.accounts.some(key => key)) {
        acc[key] = network
      }
    }
    return acc
